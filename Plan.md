...
- [x] `spd.ts`: Short Payment Descriptor ("QR Platba", Czech Banking Association) — `SPD*1.0*ACC:<iban>[+<bic>]*AM:23.50*CC:CZK*MSG:<ref>`. **CZK only.** The `CC` field takes any ISO 4217 code on paper, but in practice this is read by Czech banking apps paying in korunas, and a code that looks scannable but isn't is worse than none — the payer finds out at the bank. `bankqr.ts` picks the standard from the debt's currency (EUR → EPC, CZK → SPD, otherwise none), so the pay-up sheet and the profile preview cannot disagree, and there is nothing for the user to choose. Not attempted: **PAY by square** (SK — LZMA-compressed binary, a compression dependency for one country), **Swiss QR-bill** (designed as a payment slip, not a standalone code), **Bezahlcode** (DE — superseded by EPC).
...
- [x] README with screenshots (`docs/screenshots/`, regenerated by `scripts/screenshots.mjs`).
- [ ] Submit to the webxdc app store: a PR adding an `xdcget.ini` section on
      [codeberg.org/webxdc/xdcget](https://codeberg.org/webxdc/xdcget), which feeds the `xstore` bot.
      Requires the GitHub repo to be public and a tagged release carrying the `.xdc`. The
      ready-to-paste entry is in the README; submitting is a manual step.
...
- **The app-store address in §5 M4 was wrong** (2026-08-02). There is no
  `codeberg.org/webxdc/hub`; submissions are a PR against
  [`webxdc/xdcget`](https://codeberg.org/webxdc/xdcget), whose `xdcget.ini`
  feeds the `xstore` bot and the default stores. Its `SUBMIT.md` also
  constrains the entry — public repo, tagged release carrying the `.xdc`,
  `manifest.toml` + icon inside it, and a first description line of at most 30
